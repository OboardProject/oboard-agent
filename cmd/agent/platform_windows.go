...
// runUnderServiceManager connects the Agent to the Windows service control
// manager when SCM started it. A stop or shutdown request cancels the returned
// context, so the Agent stops through the same path as SIGTERM on Linux. The
// returned function must run after the Agent has stopped; it reports the
