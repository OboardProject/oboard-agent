...
type Store struct {
	...
	// It is never persisted by this store.
	key    []byte
	loaded bool
	err    error
	lease  *Lease
...
// and deny watermark as encrypted envelopes; an empty key keeps plaintext
// JSON for compatibility with the standard layout.
...
func (s *Store) readState(path string) ([]byte, error) {
	// #nosec G304 -- path is the store path derived from the agent state directory.
