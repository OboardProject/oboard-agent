...
var _ runtimeuser.Inbound = (*Inbound)(nil)
...
func NewInbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.Hysteria2InboundOptions) (adapter.Inbound, error) {
	...
	if err := applyHysteria2Users(hysteriaService, options.Users); err != nil {
		return nil, err
	}
...
func (h *Inbound) UpdateRuntimeUsers(users []runtimeuser.Spec) error {
	converted := make([]option.Hysteria2User, 0, len(users))
	for _, user := range users {
		name := strings.TrimSpace(user.Name)
		if name == "" || strings.TrimSpace(user.Password) == "" {
			return fmt.Errorf("hysteria2 runtime user requires name and password")
		}
		converted = append(converted, option.Hysteria2User{Name: name, Password: user.Password})
	}
	return applyHysteria2Users(h.service, converted)
}

func (h *Inbound) RuntimeUsersCapability() string {
	return runtimeuser.CapabilityHysteria2
}

func applyHysteria2Users(service *hysteria2.Service[string], users []option.Hysteria2User) error {
	...
	for _, user := range users {
		name := strings.TrimSpace(user.Name)
		if name == "" {
			return fmt.Errorf("hysteria2 user name is required")
		}
		if _, ok := seen[name]; ok {
			return fmt.Errorf("duplicate hysteria2 user %q", name)
		}
		seen[name] = struct{}{}
...
func (h *Inbound) NewConnectionEx(ctx context.Context, conn net.Conn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	...
	if userName, ok := auth.UserFromContext[string](ctx); ok && userName != "" {
