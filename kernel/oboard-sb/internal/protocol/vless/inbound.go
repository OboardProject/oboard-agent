...
func NewInbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.VLESSInboundOptions) (adapter.Inbound, error) {
	...
	if err := applyVLESSUsers(service, options.Users); err != nil {
		return nil, err
	}
...
func (h *Inbound) UpdateRuntimeUsers(users []runtimeuser.Spec) error {
	converted := make([]option.VLESSUser, 0, len(users))
	for _, user := range users {
		name := strings.TrimSpace(user.Name)
		if name == "" || strings.TrimSpace(user.UUID) == "" {
			return fmt.Errorf("vless runtime user requires name and uuid")
		}
		converted = append(converted, option.VLESSUser{Name: name, UUID: user.UUID, Flow: user.Flow})
	}
	return applyVLESSUsers(h.service, converted)
}

func (h *Inbound) RuntimeUsersCapability() string {
	return runtimeuser.CapabilityVLESS
}

func applyVLESSUsers(service *vless.Service[string], users []option.VLESSUser) error {
	...
	for i, user := range users {
		name := strings.TrimSpace(user.Name)
		if name == "" {
			return fmt.Errorf("vless user name is required")
		}
		if _, ok := seen[name]; ok {
			return fmt.Errorf("duplicate vless user %q", name)
		}
		seen[name] = struct{}{}
		names[i] = name
		uuids[i] = user.UUID
	...
	service.UpdateUsers(names, uuids, flows)
	return nil
}
